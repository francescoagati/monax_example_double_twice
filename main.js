...
Collection.prototype.objs = null;
Collection.prototype.all = function(name,cb) {
	var me = this;
	haxe.Timer.delay(function() {
		cb(null,me.objs);
	},1000);
}
Collection.prototype.__class__ = Collection;
DB = function(p) {
	if( p === $_ ) return;
	this.coll = new Collection();
}
DB.__name__ = ["DB"];
DB.prototype.coll = null;
DB.prototype.collection = function(name,cb) {
	var me = this;
	haxe.Timer.delay(function() {
		cb(null,me.coll);
	},1000);
}
DB.prototype.__class__ = DB;
Main = function() { }
Main.__name__ = ["Main"];
Main.main = function() {
	var db = new DB();
	var res5 = "";
	(NodeM.flatMap(function(_) {
		return db.collection("avatars",_);
	},function(coll) {
		return NodeM.map(function(_) {
			return coll.all("ssss",_);
		},function(avatars) {
			return avatars.length;
		});
	}))(function(err,res) {
		haxe.Log.trace(res,{ fileName : "Main.hx", lineNumber : 54, className : "Main", methodName : "main"});
	});
}
...
NodeM.prototype.__class__ = NodeM;
...
if(!com.mindrocks) com.mindrocks = {}
if(!com.mindrocks.monads) com.mindrocks.monads = {}
com.mindrocks.monads.Option = { __ename__ : ["com","mindrocks","monads","Option"], __constructs__ : ["None","Some"] }
